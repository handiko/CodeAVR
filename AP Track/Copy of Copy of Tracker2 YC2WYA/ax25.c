...
*/
	...
	ax25sendEEPROMString(0);				// Send the header for use on 144.80 MHz


	// Select the right SSID for the desired use
	if ((PIND & SW_CAR_MOTORBIKE) == SW_CAR_MOTORBIKE)
	{	// SSID for Icon 1
		ax25sendByte(eeprom_read_byte ((uint8_t *)(APRS_SSID_ADR_1)));
	}
	else
	{	// SSID for Icon 2
		ax25sendByte(eeprom_read_byte ((uint8_t *)(APRS_SSID_ADR_2)));
	}


//	ax25sendByte(APRS_SSID_DEFAULT);		// Send SSID 0 for general use

	ax25sendEEPROMString(14);				// Send the rest of the header
	return;
