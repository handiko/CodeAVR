...
void base91_long(void)
{
  	float deg;
        ...
        deg = ((posisi_long[0]-48)*100) + ((posisi_long[1]-48)*10) + ((posisi_long[2]-48)*1);
        min = ((posisi_long[3]-48)*10) + (posisi_long[4]-48);
        //sec = ((posisi_long[6]-48)*1000.0) + ((posisi_long[7]-48)*100.0) + ((posisi_long[8]-48)*10.0) + (posisi_long[9]-48); 	// <---- 1 foot resolution
        sec = ((posisi_long[6]-48)*100.0) + ((posisi_long[7]-48)*10.0) + ((posisi_long[8]-48)*1.0);				// <---- 1 meter resolution
        //sec = ((posisi_long[6]-48)*10.0) + (posisi_long[7]-48);  								// <---- 8 meter resolution

        if(posisi_long[8]=='E') sign = 1.0;
        else			sign = -1.0;

        //f_lon = (deg + (min/60.0) + (0.6*sec/360000.0));	// <---- 1 foot resolution
        f_lon = (deg + (min/60.0) + (0.6*sec/36000.0)); 	// <---- 1 meter resolution
        //f_lon = (deg + (min/60.0) + (0.6*sec/3600.0));	// <---- 8 meter resolution
...
void kirim_paket(void)
{
	char i;
        ...
        base91_long();
        base91_alt();
        //altitude+=3;             	// <---------- incremental of altitude
        //if(altitude>65500)altitude=0;	// <---------- altitude limiter
        ...
        if((beacon_stat==1)||((beacon_stat%m_int)!=0))
        {
        	kirim_add_aprs();
        	kirim_ax25_head();
...
void mem_display(void)
{
	// configuration ECHO
        ...
        putchar(9);putchar(9);putchar(9);putchar(9);putchar(9);putchar(':');
        for(k=0;k<10;k++)putchar(posisi_lat[k]);
        putchar(',');
        for(k=0;k<11;k++)putchar(posisi_long[k]);
        	...
        	if(getchar() == 'G')
        	{
        		if(getchar() == 'G')
        		{
        				...
        				HIJAU = 1;
                                	...
                                	waitComma();
                                	waitComma();
					for(i=0; i<9; i++)	posisi_lat[i] = getchar();
					getchar();
					posisi_lat[9] = getchar();
					getchar();
					for(i=0; i<10; i++)	posisi_long[i] = getchar();
					getchar();
                                        posisi_long[10] = getchar();
					waitComma();
                                	waitComma();
                                	waitComma();
                                	waitComma();
