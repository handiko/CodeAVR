...
if(adc_result2[1]>adc_tres2[1]||adc_result2[2]>adc_tres2[2]||adc_result2[3]>adc_tres2[3])rear_kanan=1;
else if(adc_result2[1]>adc_tres2[1]||adc_result2[2]>adc_tres2[2]||adc_result2[3]>adc_tres2[3])rear_kanan=0;

if(adc_result2[6]>adc_tres2[6]||adc_result2[5]>adc_tres2[5]||adc_result2[4]>adc_tres2[4])rear_kiri=1;
else if(adc_result2[6]>adc_tres2[6]||adc_result2[5]>adc_tres2[5]||adc_result2[4]>adc_tres2[4])rear_kiri=0;
}
...
switch(front_sensor)
    {
    case 0b11111000:error=  10;white_line=1;break;
    case 0b11110001:error=   5;white_line=1;break;
    case 0b11100011:error=   1;kp=speed/20;kd=6*kp;break;
    case 0b11000111:error=  -1;kp=speed/20;kd=6*kp;break;
    case 0b10001111:error=  -5;white_line=1;break;
    case 0b00011111:error= -10;white_line=1;break;
    }                           
}

void per4an_handler()
{  
   if(per4an>=9) goto selesai;
   per4an++;
...
switch(front_sensor)
        {
        case 0b01000010:fork_Y();break;
        case 0b10000001:fork_Y();break;
        case 0b11000011:fork_Y();break;
        case 0b10000011:fork_Y();break;
        case 0b11000001:fork_Y();break;
        case 0b11000110:fork_Y();break;
        case 0b01100011:fork_Y();break;
        case 0b01000011:fork_Y();break;
        case 0b01100010:fork_Y();break;
        }
}
    ...
    switch (sekarang)
            ...
            case 0: rem();
                    ...
                    for(;;)
                        ...
                        delay_us(700);
                        read_sensor();
                        if(depan_kiri==0b10000000)break;
                        };
                    backlight=0;
                    break;
            case 1: rem();
                    speed_ka=-150;speed_ki=150;pwm_out(); 
                    //speed_ka=-speed;speed_ki=speed/2;pwm_out();
                    for(;;)
                        {
                        delay_us(700);
                        read_sensor();
                        if(depan_kanan==0b0000000)break;
                        };
                    backlight=0;
                    break;
            case 2:lurus();break;
...
else if (right_back==0){right_back=0;}

if(left_back>0){left_back--;}
else if (left_back==0){left_back=0;}
...
if (enter==0){if (fork_status>=1) fork_status=0; else fork_status++;delay_ms(300);} //set cabang Y
scan_Y(); //cabang Y
...
/*----------percabangan 3 dan 4-----------*/
          if  (rear_sensor==0b00010000||rear_sensor==0b00110000||rear_sensor==0b00100000)
            {
               cross_handler();
            }
        else if (rear_sensor==0b00001000||rear_sensor==0b00001100||rear_sensor==0b00000100)
            {
                cross_handler();
            }
        else if(rear_sensor==0b00011000||rear_sensor==0b00100100||rear_sensor==0b00111100||rear_sensor==0b00101100||rear_sensor==0b00110100)
                {cross_handler();}
        
    

 /*-----------deteksi siku dan sudut 45 pake sensor belakang ----------*/
 if(front_sensor==0||front_sensor==0b01100000||front_sensor==0b00000110)
        {
...
if(adc_menu<25)
    ...
    {
    lcd_putsf("6. Arah Per4an 6");
    set_per4an=6;
    }    
else if(adc_menu<175)
    {
    lcd_putsf("7. Arah Per4an 7");
    set_per4an=7;
    }
else if(adc_menu<200)
    {
    lcd_putsf("8. Arah Per4an 8");
    set_per4an=8;
    }
else if(adc_menu<225)
    {
    lcd_putsf("9. Arah Per4an 9");
    set_per4an=9;
    }
else{
    lcd_putsf("10. Arah Per4an 10");
    set_per4an=10;
    }
...
else if(set_per4an==6) goto per4an_6;
else if(set_per4an==7) goto per4an_7;
else if(set_per4an==8) goto per4an_8;
else if(set_per4an==9) goto per4an_9;
else if(set_per4an==10) goto per4an_10;
...
per4an_6:
    ...
    if         (adc_menu<=50) per4an_dir[6]=0;
    else if  (adc_menu<=120)per4an_dir[6]=1;
    else                             per4an_dir[6]=2;
    
    lcd_gotoxy(0,1);
    if     (e_per4an_dir[6]==0)lcd_putsf("left");
    else if(e_per4an_dir[6]==1)lcd_putsf("right");
    else if(e_per4an_dir[6]==2)lcd_putsf("lurus");
    
    lcd_gotoxy(7,1);
    lcd_putsf("<-");
    if     (per4an_dir[6]==0)lcd_putsf("left");
    else if(per4an_dir[6]==1)lcd_putsf("right");
    else if(per4an_dir[6]==2)lcd_putsf("lurus");
    
    if(enter==0){e_per4an_dir[6]=per4an_dir[6];per4an_dir[6]=per4an_dir[6];delay_ms(400);lcd_clear();goto awal;}
    if(back==0){delay_ms(400);lcd_clear();goto awal;}
    delay_ms(50);
    }
    ...
    if         (adc_menu<=50) per4an_dir[7]=0;
    else if  (adc_menu<=120)per4an_dir[7]=1;
    else                             per4an_dir[7]=2;
    
    lcd_gotoxy(0,1);
    if     (e_per4an_dir[7]==0)lcd_putsf("left");
    else if(e_per4an_dir[7]==1)lcd_putsf("right");
    else if(e_per4an_dir[7]==2)lcd_putsf("lurus");
    
    lcd_gotoxy(7,1);
    lcd_putsf("<-");
    if     (per4an_dir[7]==0)lcd_putsf("left");
    else if(per4an_dir[7]==1)lcd_putsf("right");
    else if(per4an_dir[7]==2)lcd_putsf("lurus");
    
    if(enter==0){e_per4an_dir[7]=per4an_dir[7];per4an_dir[7]=per4an_dir[7];delay_ms(400);lcd_clear();goto awal;}
    if(back==0){delay_ms(400);lcd_clear();goto awal;}
    delay_ms(50);
    }
    ...
    if         (adc_menu<=50) per4an_dir[8]=0;
    else if  (adc_menu<=120)per4an_dir[8]=1;
    else                             per4an_dir[8]=2;
    
    lcd_gotoxy(0,1);
    if     (e_per4an_dir[8]==0)lcd_putsf("left");
    else if(e_per4an_dir[8]==1)lcd_putsf("right");
    else if(e_per4an_dir[8]==2)lcd_putsf("lurus");
    
    lcd_gotoxy(7,1);
    lcd_putsf("<-");
    if     (per4an_dir[8]==0)lcd_putsf("left");
    else if(per4an_dir[8]==1)lcd_putsf("right");
    else if(per4an_dir[8]==2)lcd_putsf("lurus");
    
    if(enter==0){e_per4an_dir[8]=per4an_dir[8];per4an_dir[8]=per4an_dir[8];delay_ms(400);lcd_clear();goto awal;}
    if(back==0){delay_ms(400);lcd_clear();goto awal;}
    delay_ms(50);
    }
    ...
    if         (adc_menu<=50) per4an_dir[9]=0;
    else if  (adc_menu<=120)per4an_dir[9]=1;
    else                             per4an_dir[9]=2;
    
    lcd_gotoxy(0,1);
    if     (e_per4an_dir[9]==0)lcd_putsf("left");
    else if(e_per4an_dir[9]==1)lcd_putsf("right");
    else if(e_per4an_dir[9]==2)lcd_putsf("lurus");
    
    lcd_gotoxy(7,1);
    lcd_putsf("<-");
    if     (per4an_dir[9]==0)lcd_putsf("left");
    else if(per4an_dir[9]==1)lcd_putsf("right");
    else if(per4an_dir[9]==2)lcd_putsf("lurus");
    
    if(enter==0){e_per4an_dir[9]=per4an_dir[9];per4an_dir[9]=per4an_dir[9];delay_ms(400);lcd_clear();goto awal;}
    if(back==0){delay_ms(400);lcd_clear();goto awal;}
    delay_ms(50);
    }
    ...
    if         (adc_menu<=50) per4an_dir[10]=0;
    else if  (adc_menu<=120)per4an_dir[10]=1;
    else                             per4an_dir[10]=2;
    
    lcd_gotoxy(0,1);
    if     (e_per4an_dir[10]==0)lcd_putsf("left");
    else if(e_per4an_dir[10]==1)lcd_putsf("right");
    else if(e_per4an_dir[10]==2)lcd_putsf("lurus");
    
    lcd_gotoxy(7,1);
    lcd_putsf("<-");
    if     (per4an_dir[10]==0)lcd_putsf("left");
    else if(per4an_dir[10]==1)lcd_putsf("right");
    else if(per4an_dir[10]==2)lcd_putsf("lurus");
    
    if(enter==0){e_per4an_dir[10]=per4an_dir[10];per4an_dir[10]=per4an_dir[10];delay_ms(400);lcd_clear();goto awal;}
    if(back==0){delay_ms(400);lcd_clear();goto awal;}
    delay_ms(50);
    }
...
fork_status=0; // 0 kiri, 1 kanan, 2 lurus
per4an=1;
e_per4an_enable=1;
